...
**Version 4.3 — Production-Grade Buildable SRS**
**Date:** 2026-06-25
...
|---------|------|--------|---------|
| 4.3 | 2026-06-25 | tonmoy | Added FR-HD-007 (Guarded Environment Auto-Remediation — `forge doctor --fix`): opt-in mutator that repairs the FR-HD-006 FAIL checks it can safely fix (create virtualenv, install missing dependencies, `forge init` a missing project, rebuild an invalid `config.yaml`). Guarded — shows a dry-run plan, confirms each action, refuses CI/no-TTY without `--yes`, requires `--force` to re-`init` over an existing `.forge/`, rewrites config only when already invalid, and audits the rewrite (`doctor_autofix_config_rewrite` / `ALLOWED`); each applied fix re-runs its FR-HD-006 check. Bare `forge doctor` stays read-only. Filename held stable (`SRSv4.1.md`); this changelog table is the authoritative version record. |
...
| FR-HD-007 | **[v4.3] Guarded Environment Auto-Remediation** — `forge doctor --fix` is an opt-in mutator that repairs the FR-HD-006 FAIL checks it can safely fix: create a virtualenv, install missing dependencies, `forge init` a missing project, and rebuild an invalid `config.yaml`. Bare `forge doctor` stays **read-only** (FR-HD-006); mutation happens only under `--fix`. It is a **guarded mutator** — it prints a dry-run plan of intended actions before applying any, confirms each action individually, applies non-destructive defaults, and refuses to act in a non-interactive environment (no TTY / CI) unless `--yes` is given. Config rebuild fires **only when `config.yaml` is already invalid** (a valid config is never rewritten); re-`init` over an existing `.forge/` requires `--force`. The config rewrite is audited via the project `SecurityAuditLog` (`action="doctor_autofix_config_rewrite"`, `decision=ALLOWED`); pre-project fixes (venv/deps/init) run before a project exists and therefore **disclose their actions in output** rather than writing to a not-yet-existent project audit sink. Distinct from FR-HD-006 (read-only reporting). | Bare `forge doctor` performs no mutation — only `--fix` mutates; `--fix` prints a dry-run plan before applying anything; each remediable FAIL is confirmed individually and declining skips only that action; in no-TTY/CI `--fix` refuses to act without `--yes`; config rebuild triggers only when `load_config` reports the config invalid; re-`init` over an existing `.forge/` is skipped unless `--force` is passed; the config rewrite emits a `SecurityAuditLog` entry `doctor_autofix_config_rewrite` / `ALLOWED`; after each fix the corresponding FR-HD-006 check is re-run and its new status reported, and `--fix` exits non-zero if any required check still FAILs. |
