...
### L011 — `load_config` can raise TWO unrelated `ConfigError` classes; a best-effort hook in the spawn path must catch broadly
- **Date:** 2026-06-24
- **Trigger:** The per-session token monitor (FR-HD-003) wired into `run_stage_agent_async` caught `config.loader.ConfigError` only. A config with a whitespace `schema_version` made `load_config` raise `schemas.config.ConfigError` instead, which escaped and **broke the async spawn** — violating the helper's own "never break a spawn" contract. Caught by adversarial review (reproduced end-to-end).
- **Root cause:** Two unrelated `ConfigError` hierarchies exist: `config.loader.ConfigError(RuntimeError)` (raised by the loader for bad/missing/invalid YAML) and `schemas.config.ConfigError(Exception)` (raised by `ForgeConfig` `field_validator`s). The schemas one is **not** a `ValueError`, so Pydantic does **not** wrap it in `ValidationError`; it bypasses `load_config`'s `except ValidationError` and propagates raw. A narrow `except ConfigError` (loader variant) misses it. The async executor path is the live hazard because the monitor is its *only* `load_config` call.
- **Rule:** A best-effort/observability hook wired into the spawn or any critical path MUST degrade every failure to a log line and never propagate — at an IO/config boundary catch broadly (`except Exception` with a logged reason), not a specific type. Concretely: do not assume `load_config` raises only `config.loader.ConfigError`; it can also raise the validator-side `schemas.config.ConfigError` (and `OSError`). When catching to protect an absolute "never break X" contract, breadth beats precision.
- **Files affected:** `src/forge_os/agents/executor.py` (`_monitor_token_budget`), `tests/test_agents_executor_token_monitor.py`.
